...
const OPS_RULES: &str = "\
## ルール
- 作業手順を参考にしつつ、依頼の意図を読み取って柔軟に対応すること
- 手順に明記されていない作業でも、リポジトリへの変更で対応できるなら実行すること
- 不明な点があれば作業を中断し、確認が必要な内容を報告すること
- 対応不要なのは、リポジトリへの変更を伴わないメッセージのみ（障害報告、雑談など）

## YAGNI（余計なことをするな）
- 頼まれていない機能追加・リファクタリング・最適化をしない
- 変更していないコードにコメントやドキュメントを追加しない
- 起こり得ないシナリオのエラーハンドリングを書かない
- 3行の類似コードは早すぎる抽象化より良い
- 仮説的な将来要件のために設計しない
- バグ修正に周辺コードの整理は不要

## 自動実行 vs 提案
- 作業手順に記載されている定型 ops（サブカテ追加、画像変更、クローズ対応など）→ そのまま実行
...
出力は以下の **2 セクション構造** にすること:

### サマリ（必須）
`## サマリ` 見出しの後に 2-3 行で結果概要を書く。
依頼者（非エンジニア）がすぐ理解できる内容にする。

### 詳細（任意）
`## 詳細` 見出しの後に技術的な詳細（変更ファイル、確認結果など）を書く。
軽微な作業（typo 修正など）では省略してよい。

### ステータスマーカー（必須）
出力の最終行に必ず以下のマーカーを付けること（システムが自動解析する）:
- 作業を実行した場合: `OPS_RESULT: completed`
- 対応不要の場合: `OPS_RESULT: no_action`（理由も添えること）
- 定型外の依頼で提案のみ行った場合: `OPS_RESULT: proposal`
- 失敗した場合: `OPS_RESULT: failed`

例:
```
## サマリ
管理No.269: サブカテ4件追加、5件クローズ完了。即時利用可能。

## 詳細
- image_mappings.yaml L294-297: ANGEL REPORT / 奥義ノ矢取材 / 風神取材 / 雷神取材 追加
- images/269_182-185.jpg 配置（82-174KB）
- L106/118/143/204/235: 5件コメントアウト（クローズ処理）
- git push 済み
